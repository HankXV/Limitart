...
# 游戏服务器开发思路
1.首先你需要对接好网络通信<br>
2.根据游戏的不同来制定线程模型，在游戏中制定线程模型基本抱着两个目的。第一，IO或者复杂计算不能阻塞玩家的操作。第二，如果玩家有数据交互，他们应当在同一个线程<br>
3.根据2的参考方式制定了线程模型，然后在服务器的`dispatchMessage`回调方法中发消息分发到不同线程中<br>
4.如果你不做分发，直接在`dispatchMessage`中执行handler，那么就默认使用了Netty的work线程，他只保证了一个channel一定会在同一个线程运行<br>
