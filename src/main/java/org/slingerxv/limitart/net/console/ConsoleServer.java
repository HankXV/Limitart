...
public class ConsoleServer extends AbstractNettyServer implements IServer {
	...
	private class ChannelInitializerImpl extends ChannelInitializer<SocketChannel> {
		...
		protected void initChannel(SocketChannel ch) {
			ch.pipeline().addLast(new StringDecoder(CharsetUtil.UTF_8)).addLast(new StringEncoder(CharsetUtil.UTF_8))
						...
						public void channelRead(ChannelHandlerContext ctx, Object msg) {
							...
							if (consoleUser == null) {
								...
								if (tempUsername == null) {
									...
									try {
										if (!SecurityUtil.isPasswordValid(temp.getPass(), command,
												temp.getUsername())) {
